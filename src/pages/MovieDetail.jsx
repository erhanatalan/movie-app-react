...
const MovieDetail = () => {
  ...
  const {title, poster_path, overview, vote_count, release_date, vote_average} = movieDetail;
  ...
  useEffect(() => {
    ...
    axios.get(videoUrl).then((res) => setVideoKey(res.data.results[0].key)).catch(err=>console.log(err))
  ...
  return (
    ...
    {videoKey && <VideoSection videoKey={videoKey} videoUrl={videoUrl} />}
